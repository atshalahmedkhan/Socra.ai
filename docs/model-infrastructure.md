...
Copy `.env.example` to `.env`, generate strong model API keys, accept Gemma terms using an authorized Hugging Face account, and run `python model/benchmarks/scripts/verify_gemma_access.py`. A full weight download requires the explicit `--download-model` flag.

Start with `docker compose -f docker-compose.yml -f docker-compose.gpu.yml up --build`. Test a reachable server with `python model/benchmarks/scripts/test_model_endpoint.py --base-url URL --model gemma-3-4b-base`. Model ports use `expose`, not host publishing.
...
Health routes are `/health/live`, `/health/ready`, and the default-disabled `/health/model`. Benchmarks use `model/benchmarks/scripts/benchmark_model.py` and `model/benchmarks/scripts/run_benchmark_matrix.py`.
